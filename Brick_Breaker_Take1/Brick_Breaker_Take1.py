...
#Function used to populate th blocks
def populateBlocks(blockWidth, blockHeight, horizontalGap, verticalGap):
    listOfBlocks = []
    
    for i in range(0, HEIGHT//2, blockHeight+horizontalGap):
        for j in range(0, HEIGHT//2, blockHeight+verticalGap):
            listOfBlocks.append(BLOCKSIZE(i, j, blockWidth,
                                      blockHeight,
...
class Striker:
    ...
    class Ball:
        ...
        def display(self):
            self.ball = pygame.draw.circle(screen, self.color, (self.posx, self.posy))
        ...
        def update(self):
            ...
            # Reflecting th ball if it touches either of the vertical edges
    ...
    def main():
        ...
        scoreTextRect.center = (120, HEIGHT-10)
        ...
        while running:
            screen.fill(BLACK)
            screen.bilt(scoreText, scoreTextRect)
            screen.bilt(livesText, livesTextRect)
